...
// Gated before the log-row insert and consume_sms_credit call below, so a
// disabled deployment never touches credit or writes an orphan queued row.
dispatchManualRouter.post(
  '/internal/dispatch/manual',
  ...
  async (req, res) => {
    ...
    const balance =
      (consumeResult as { credit_balance_after?: number } | null)?.credit_balance_after ?? 0;
    const replay =
      (consumeResult as { idempotent_replay?: boolean } | null)?.idempotent_replay ?? false;
